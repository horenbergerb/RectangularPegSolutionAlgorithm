...
ax1.set_title("z, w (Green, Black)")
ax1.plot(data[:,0], data[:,1])


    
#L plot
ax2.set_title("p,q (Green, Black) for 1st Rectangle")
#an overlay of the original just to intuitively see what's goin on
ax2.plot(data[:,0], data[:,1])
ax2.set_xlim([np.amin(sample_L[:,2])-10, np.amax(sample_L[:,0])+50])
ax2.set_ylim([np.amin(sample_L[:,3])-10, np.amax(sample_L[:,1])+50])

#L_phi plot
ax3.set_title("p,q (Green, Black) for 2nd Rectangle")
#an overlay of the original just to intuitively see what's goin on
ax3.plot(data[:,0], data[:,1])
ax3.set_xlim([np.amin(sample_L_phi[:,2])-10, np.amax(sample_L_phi[:,0])+50])
ax3.set_ylim([np.amin(sample_L_phi[:,3])-10, np.amax(sample_L_phi[:,1])+50])

#rectangle from L points
ax4.set_title("First Rectangle")
ax4.plot(data[:,0], data[:,1])

ax5.set_title("Second Rectangle")
ax5.plot(data[:,0], data[:,1])

#the two points on lambdaXlambda
sample1_pt1, = ax1.plot(sample_pts1[0][0],sample_pts1[0][1], 'go')
sample1_pt2, = ax1.plot(sample_pts1[0][2],sample_pts1[0][3], 'ko')
#L term 1
L1, = ax2.plot(sample_L[0][0], sample_L[0][1], 'go')
#L term 2
L2, = ax2.plot(sample_L[0][2], sample_L[0][3], 'ko')
#L_phi term 1
L_phi1, = ax3.plot(sample_L_phi[0][0], sample_L_phi[0][1], 'go')
#L_phi term 2
L_phi2, = ax3.plot(sample_L_phi[0][2], sample_L_phi[0][3], 'ko')
...
ani = animation.FuncAnimation(fig, update, sample_pts1.shape[0], fargs=(sample1_pt1, sample1_pt2, L1, L2, L_phi1, L_phi2, L_rectangle, L_phi_rectangle,), blit=False)
