...
filename = 'rectanglecompleted.png'
...
cur_rec = get_rectangle_pts(phi, copy.deepcopy(sample_L[30]))
print(cur_rec)
L_rectangle, = axs[1].plot(cur_rec[:,0], cur_rec[:,1], 'go')

cur_rec = get_rectangle_pts(phi, copy.deepcopy(sample_L_phi[30]))
print(cur_rec)
L_phi_rectangle, = axs[2].plot(cur_rec[:,0], cur_rec[:,1], 'go')
